...
interface ContactCollaborationProps {
  contact: Contact;
}

const ContactCollaboration = ({ contact }: ContactCollaborationProps) => {
  const { user } = useAuth();
    ...
    const loadData = async () => {
      try {
        const [commentsData, remindersData, interactionsData] =
          await Promise.all([
            collaborationService.getCommentsByEntity("contact", contact.id!),
            collaborationService.getRemindersByEntity("contact", contact.id!),
