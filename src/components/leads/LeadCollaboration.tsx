...
import { Lead } from "@/lib/services/firebase/leads";
import {
...
interface LeadCollaborationProps {
  lead: Lead;
}

const LeadCollaboration = ({ lead }: LeadCollaborationProps) => {
  const { user } = useAuth();
    ...
    const loadData = async () => {
      try {
        const [commentsData, remindersData, interactionsData] =
          await Promise.all([
            collaborationService.getCommentsByEntity("lead", lead.id!),
            collaborationService.getRemindersByEntity("lead", lead.id!),
            collaborationService.getInteractionsByEntity("lead", lead.id!),
          ]);
  ...
  const handleAddComment = async () => {
    ...
    try {
      const comment = await collaborationService.addComment("lead", lead.id!, {
        content: newComment,
        userId: user.id!,
  ...
  const handleAddInteraction = async () => {
    ...
    try {
      ...
      // Update the lead with the last interaction information
      await leadsService.update(lead.id!, {
        lastInteractionBy: user.displayName,
        lastInteractionDate: new Date().toISOString(),
        nextContactDate: interactionForm.followUpDate || lead.nextContactDate,
      });
...
export default LeadCollaboration;
